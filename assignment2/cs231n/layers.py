...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_normalized = x - sample_mean
    ivar = (sample_var + eps)**(-0.5)
    x_normalized = x_normalized * ivar
    
    #x_normalized = (x-sample_mean) / np.sqrt(sample_var + eps) # this work too
    out = gamma*x_normalized + beta
    
    #out = xhat*gamma + beta
    #cache = (x, xhat, gamma, sample_mean, sample_var, eps)
...
def batchnorm_backward(dout, cache):
  ...
  dsqure = divar * (-0.5)* (sample_var + eps)**(-1.5)/N
  dxmu2 = 2*xmu * dsqure
  dxmu = dxmu1 + dxmu2
  dx1 = dxmu
  dx2 = np.sum(-dxmu, axis = 0)/N
  dx = dx1 + dx2
